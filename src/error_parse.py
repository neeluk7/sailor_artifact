...
def classify(total, illegal, timeouts):
    ...
    if total_failures < total:
        return "Conditional"
    if total_failures == total:
        # Explicitly flag timeouts
        if timeouts == total:
            return "Timeout"
        elif timeouts > 0:
            return "Timeout / Not allowed"
        else:
            return "Not allowed"
...
def parse_traces(trace_files, sysregs):
    footprint = [{}, {}, {}, {}]
    access    = [{}, {}, {}, {}]
    throws    = [{}, {}, {}, {}]  # NEW: Tracks throw_at reasons per EL
    ...
    for filename in trace_files:
        ...
        for line in lines:
            ...
            if "--- Instruction:" in line:
                # Clean up trailing dashes as well so they don't pollute the throw string
                clean_content = line.replace("--- Instruction:", "").replace("---", "").strip()
                ...
                # NEW: Extract the throw_at reason
                for part in pipe_parts:
                    if part.startswith("throw_at:"):
                        throws[el][instr] = part.replace("throw_at:", "").strip()
            ...
            # ── ONLY rule for an illegal trace: Sail Error_Undefined ─────────
    ...
    return footprint, access, throws


def write_csvs(footprint, access, throws):
    ...
    throws_file = os.path.join(OUTPUT_DIR, "instruction_throws.csv")  # NEW output file
    ...
    # NEW: Throws CSV (Filtered for exceptions only)
    with open(throws_file, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(["Instruction", "EL0 Throw", "EL1 Throw", "EL2 Throw", "EL3 Throw"])
        for instr in all_instrs:
            row = [throws[el].get(instr, "") for el in range(4)]
            if any(row):  # Only output if at least one EL recorded a throw_at
                w.writerow([instr] + row)
...
if __name__ == "__main__":
    ...
    # Ensure unpacking handles the newly returned 'throws' dictionary
    footprint, access, throws = parse_traces(ISLA_TRACE_FILES, sysregs)
    write_csvs(footprint, access, throws)
