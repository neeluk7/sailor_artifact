...
def parse_tracesbis(trace_files, sysregs):
    ...
    for filename in trace_files:
        # ---- EL from filename (reliable; one EL per file) -----------------
        el = next(
            (i for i, tok in enumerate(["el0", "el1", "el2", "el3"])
             if tok in filename.lower()),
            -1,
        ...
        def commit():
            """Write classification for the current instruction block."""
            if instr:
                access[el][instr] = classify(total_traces, illegal_traces)
                print(total_traces, illegal_traces)
        ...
        for line in lines:
            ...
            # -- New instruction block ---------------------------------------
            if "INSTRUCTION CONSTRUCTED:" in line:
                commit()                    # save previous instruction
                total_traces   = 0
                illegal_traces = 0

                instr = line.split(":")[1].strip()
                for i in range(4):          # pre-populate all ELs so write_csvs
                    footprint[i].setdefault(instr, [])  # can always find the key


            elif "(trace" in line:
                trace_is_illegal = False          # open next trace clean
                total_traces += 1

 
            # -- Illegal instruction (ESR write with EC == 0) ---------------
            elif any(r in line for r in ESR_REGS):
                if _is_illegal_esr(line):
                    if trace_is_illegal == False :
                        trace_is_illegal = True
                        illegal_traces += 1
                        print("Type 1 baddy ")


            elif "(write-reg |SEE|" in line:
                print(f"  [DEBUG] SEE in {instr}: {line.strip()}")
                # -1 (bv-1) clears the exception. Anything else (like bv203) is a trap.
                if "bv-1 " not in line:
                    if trace_is_illegal == False :
                        trace_is_illegal = True
                        illegal_traces += 1
            ...
            # -- Register read (whole register, not a field access) ---------
            elif "read-reg" in line and "field" not in line:
                parts = line.split("|")
                if len(parts) >= 2:
                    reg = parts[1]
                    if reg in sysregs and instr:
                        fp = footprint[el][instr]
                        # Upgrade "Read" to "Write" if we later see a write;
                        # never add a Read entry when a Write entry already exists.
                        if reg + " Read" not in fp and reg + " Write" not in fp:
                            fp.append(reg + " Read")
 
            # -- Register write (whole register, not a field access) --------
            elif "write-reg" in line and "field" not in line:
                parts = line.split("|")
                if len(parts) >= 2:
                    reg = parts[1]
                    if reg in sysregs and instr:
                        fp = footprint[el][instr]
                        if reg + " Write" not in fp:
                            # Replace a prior Read entry with Write
                            read_key = reg + " Read"
                            if read_key in fp:
                                fp[fp.index(read_key)] = reg + " Write"
                            else:
